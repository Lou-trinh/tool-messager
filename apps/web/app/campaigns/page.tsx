...
type Contact = { id: string; displayName: string; consentStatus: string; suppressed: boolean; platformUserId?: string };
...
export default function CampaignsPage() {
  ...
  const [form, setForm] = useState({ name: '', accountId: '', templateId: '', contactIds: [] as string[], promotional: true });
  ...
  const create = useMutation({ mutationFn: () => api<Campaign>(`/workspaces/${workspaceId}/campaigns`, { method: 'POST', body: JSON.stringify(form) }), onSuccess: async (created) => { setForm({ name: '', accountId: '', templateId: '', contactIds: [], promotional: true }); setSelectedCampaign(created.id); setError(''); await refresh(); }, onError: (cause: Error) => setError(cause.message) });
  ...
  const eligible = contacts.data?.items.filter((contact) => !contact.suppressed && (!form.promotional || contact.consentStatus === 'OPTED_IN')) ?? [];
  ...
  return <AppShell title="Chiến dịch tin nhắn" subtitle="Tạo → preview consent → phê duyệt → queue → worker. Không có gửi đồng bộ trong request API.">
    ...
    <form className="panel mb-5 grid gap-4 p-5" onSubmit={(event: FormEvent) => { event.preventDefault(); create.mutate(); }}>
      ...
      <div className="grid gap-3 md:grid-cols-3"><input className="input" required placeholder="Tên chiến dịch" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /><select className="input" required value={form.accountId} onChange={(event) => setForm({ ...form, accountId: event.target.value })}><option value="">Chọn Zalo OA đã kết nối</option>{accounts.data?.filter((account) => account.status === 'CONNECTED').map((account) => <option key={account.id} value={account.id}>{account.displayName}</option>)}</select><select className="input" required value={form.templateId} onChange={(event) => setForm({ ...form, templateId: event.target.value })}><option value="">Chọn template</option>{templates.data?.filter((template) => template.status === 'ACTIVE').map((template) => <option key={template.id} value={template.id}>{template.name} · v{template.version}</option>)}</select></div>
      ...
      <div className="max-h-52 overflow-auto rounded-xl border border-[var(--border)] p-3"><div className="mb-2 flex items-center justify-between text-xs text-[var(--muted)]"><span>{eligible.length} contact đủ điều kiện trong trang dữ liệu hiện tại</span><button type="button" className="text-teal-300" onClick={() => setForm({ ...form, contactIds: eligible.map((contact) => contact.id) })}>Chọn tất cả</button></div><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{eligible.map((contact) => <label className="flex items-center gap-2 rounded-lg bg-white/[.025] px-3 py-2 text-sm" key={contact.id}><input type="checkbox" checked={form.contactIds.includes(contact.id)} onChange={() => toggleContact(contact.id)} /><span className="truncate">{contact.displayName}</span><span className="ml-auto text-[10px] text-[var(--muted)]">{contact.consentStatus}</span></label>)}</div></div>
      <button className="button-primary justify-self-start" disabled={create.isPending || !form.contactIds.length}>{create.isPending ? 'Đang tạo…' : `Tạo bản nháp (${form.contactIds.length} contact)`}</button>
    </form>
