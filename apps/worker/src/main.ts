...
async function processCampaign(job: Job<{ campaignId: string }>): Promise<void> {
  ...
  for (const member of campaign.audience) {
    ...
    const syntheticRecipient = contact.source === 'SYNTHETIC_SEED';
    const validPlatformRecipient = !syntheticRecipient && contact.platform === campaign.platform && Boolean(contact.platformUserId?.trim());
    ...
    const blockedReason = syntheticRecipient ? 'SYNTHETIC_RECIPIENT' : !validPlatformRecipient ? 'INVALID_PLATFORM_RECIPIENT' : contact.suppressed || contact.status === 'DO_NOT_CONTACT' ? 'CONTACT_SUPPRESSED' : 'CONSENT_REQUIRED';
