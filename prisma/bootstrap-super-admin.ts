...
async function main(): Promise<void> {
  ...
  if (password.length < 12) throw new Error('BOOTSTRAP_SUPER_ADMIN_PASSWORD must contain at least 12 characters.');
  const existing = await prisma.user.findUnique({ where: { email }, select: { id: true } });
  const user = existing
    ? await prisma.user.update({ where: { id: existing.id }, data: { systemRole: 'SUPER_ADMIN' } })
    : await prisma.user.create({ data: { email, displayName: 'Platform Administrator', passwordHash: await argon2.hash(password), systemRole: 'SUPER_ADMIN' } });
